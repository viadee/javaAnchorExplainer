...
public class SubmodularPick<T extends DataInstance<?>> {
    ...
    public List<AnchorResult<T>> run(final List<T> instances, final int nrOfExplanationsDesired) {
        ...
        final CreateFeatureToColumnMapResult createFeatureToColumnMapResult = createFeatureToColumnMap(anchorResults);
        ...
        final double[][] importanceMatrix = new double[anchorResults.length][createFeatureToColumnMapResult.columnCount];
        for (int i = 0; i < anchorResults.length; i++) {
            for (final int feature : anchorResults[i].getOrderedFeatures()) {
                importanceMatrix[i][getCandidateFeatureIndex(featureToColumnMap, anchorResults[i], feature)] =
